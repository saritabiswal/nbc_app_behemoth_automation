		...
		try
    	{
			switch (aut.toUpperCase()) 
			{
				...
				case "CHROME":
						...
						break;
				case "SAFARI":
					lstExecutionType = objDBF.getTestDataObject("Select ExecutionType from TestSuite Where Execution ='Yes' and Browser='Safari'","TestRunner");
					if(!lstExecutionType.isEmpty())
					{
						if(lstExecutionType.get(0).compareToIgnoreCase("Remote")== 0)
						{
							poolSize=2;
							htTestRunner=objDBF.getHashTestData("Select * from TestSuite Where Execution ='Yes' AND Browser='Safari'","TestRunner");
							aut="REMOTE";
							executionCnt=htTestRunner.size();
							System.out.println("Safari:"+htTestRunner.size());
						}
						else if(lstExecutionType.get(0).compareToIgnoreCase("Grid")== 0)
						{
							htTestRunner=objDBF.getHashTestData("Select * from TestSuite Where Execution ='Yes' AND Browser='Chrome'","TestRunner");
				...
				{*/
				    ...
				    while (intIterator.hasNext()) 
				    {
					    ...
					    /* myTest.addParameter("environment", (String) lstExecutionType.get(0))*/; //Execution Type
					     myTest.addParameter("environment", aut);
					   //  System.out.println(aut);
					     myTest.addParameter("browser", (String) entry.getValue()); //Add any parameters that you want to set to the Test.
					     myTest.addParameter("version", lstBrowserVersion.get(0)); //Add any parameters that you want to set to the Test.
					     myTest.addParameter("os", lstOS.get(0)); //Add any parameters that you want to set to the Test.
					     myTest.addParameter("osversion", lstOSVersion.get(0)); //Add any parameters that you want to set to the Test.
